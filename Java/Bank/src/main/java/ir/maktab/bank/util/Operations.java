...
public class Operations {
    ...
    private static void viewListOfCustomerAccounts(Employee activeEmployee) {
        ...
        try {
            System.out.println("Enter Customer ID You Wanna Edit Accounts :");
            id = ApplicationContext.longFromScannerReturn(id);
            long finalId = id;
            ...
            Set<Account> chosenCustomerAccounts = branchAccounts.stream()
                    .filter(account -> account.getCustomer().equals(chosenCustomer))
                    .collect(Collectors.toSet());
            DetailsView.customerAccountsListView(chosenCustomerAccounts);
    ...
    public static void editCustomerAccount(Employee activeEmployee) {
        long id = 0L;
        double creditChange;
        List<Account> branchAccounts = ApplicationContext.getAccountService()
                .getAccountsByBranch(activeEmployee.getBranch());
        ...
        try {
            ...
            Account chosenAccount = chosenCustomerAccounts.stream()
                    .filter(account -> account.getAccountNumber().equals(accountNumber)).collect(Collectors.toList())
                    .get(0);

            DetailsView.oneAccountView(chosenAccount);

            System.out.println("How Much Money You Want to Change? (+ or -) : ");
            creditChange = ApplicationContext.getNumericScanner().nextDouble();
            double initCredit = chosenAccount.getCredit();

            chosenAccount.setCredit(initCredit + creditChange);

            ApplicationContext.getAccountService().save(chosenAccount);
        } catch (Exception e) {
            System.out.println("----> Wrong Id <----");
        }
    }
    ...
    private static Boolean checkCardDetails(CreditCard creditCard) {
        String cvv2;
        String secondPassword;
        String eday;
        Date expireDate;
